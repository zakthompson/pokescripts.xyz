            ...
            >
              ...
              <Form onSubmit={onSubmit}>
                {state.configFields.map((f) => {
                  ...
                  let seconds;
                  const timingStrArr = [];
                  if (f.param === 'm_dayToSkip'
                    && ['DaySkipper', 'DaySkipper_Unlimited'].includes(state.target)) {
                    totalSeconds = ((f.value/30)*dateSkipTimings[dateFormat]);
                  } else if (state.target === 'AutoFossil' && f.param === 'm_timesBeforeSR') {
                    totalSeconds = f.value * fossilReviveTime;
                  } else if (state.target === 'AutoLoto' && f.param === 'm_dayToSkip' && f.value !== 0) {
                    totalSeconds = f.value * autoLotoTime;
                  } else if (state.target === 'BoxRelease' && f.param === 'm_boxCount') {
                    totalSeconds = f.value * boxReleaseTime;
                  } else if (state.target === 'BoxSurpriseTrade' && f.param === 'm_boxesToTrade') {
                    if (state.configFields.find((f) => f.param === 'm_completeDex').value === 'true') {
                      totalSeconds = f.value * boxTradeTime;
                    } else {
                      totalSeconds = f.value * boxTradeTimeWithPokedex;
                    }
                  } else if (state.target === 'EggCollector' && f.param === 'm_maxCycle') {
                    totalSeconds = f.value * eggCollectTime;
                  } else if (state.target === 'EggHatcher' && f.param === 'm_columnsOfEggs') {
                  ...
                  if (totalSeconds) {
                    ...
                    if (hours) {
                      timingStrArr.push(`${hours} hour${hours === 1 ? '' : 's'}`);
                    }
                    if (minutes) {
                      timingStrArr.push(`${minutes} minute${minutes === 1 ? '' : 's'}`);
                    }
                    if (seconds) {
                      timingStrArr.push(`${seconds} second${seconds === 1 ? '' : 's'}`);
                    }
                  }
                      ...
                      <Box width="70%" direction="column" gap="small">
                        ...
                        {timingStrArr.length > 0 && (
                          <Box align="end" direction="row" gap="xsmall">
                            <Text size="xsmall" weight="bold">Approx. Time:</Text>
                ...
                <Box
                  direction="row"
                  justify={['Auto3DaySkipper', 'AutoLoto', 'BerryFarmer', 'WattFarmer'].includes(state.target) ? 'between' : 'end'}
                  align="center"
                  gap="medium"
                  margin={{ top: '20px' }}
                >
